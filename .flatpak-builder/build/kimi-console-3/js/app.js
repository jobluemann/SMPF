...
class App {
  ...
  switchView(view) {
    ...
    if (view === 'integrations') this.refreshIntegrations();
  }
  ...
  bindChat() {
    ...
    sel?.addEventListener('change', e => { this.provider = e.target.value; });
  ...
  async sendChat(text) {
    ...
    messages.push(...this.messageHistory.slice(-20).map(m => ({ role: m.role, content: m.content })));
    ...
    try {
      const resp = await chatCompletion(this.provider, messages, key);
  ...
  /* OpenClaw */
  async saveOpenclaw() {
    const isLocallyInstalled = document.getElementById('openclawLocal')?.checked || false;
    await Settings.set('openclawLocal', isLocallyInstalled);
    await Settings.remove('openclawKey');
    this.openclaw = new OpenClawClient(isLocallyInstalled);
    this.refreshOpenclawUI();
  }

  async loadOpenclaw() {
    const isLocallyInstalled = await Settings.get('openclawLocal', false);
    document.getElementById('openclawLocal').checked = isLocallyInstalled;
    this.openclaw = new OpenClawClient(isLocallyInstalled);
  }

  async enableOpenclaw() {
    if (!this.openclaw?.enabled) { alert('Mark OpenClaw as locally installed first.'); return; }
    const r = await this.openclaw.test();
    const status = document.getElementById('openclawStatus');
    status.textContent = r.msg;
    status.className = r.ok ? 'status-msg ok' : 'status-msg err';
  }

  refreshOpenclawUI() {
    const status = document.getElementById('openclawStatus');
    if (!status) return;
    if (this.openclaw?.enabled) { status.textContent = 'OpenClaw local installation enabled.'; status.className = 'status-msg ok'; }
    else { status.textContent = 'OpenClaw local installation not enabled.'; status.className = 'status-msg'; }
  }
